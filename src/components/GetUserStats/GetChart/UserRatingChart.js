  ...
  useEffect(() => {
    ...
    svg.on("pointerenter pointermove", (event) => {
      if (event.target.tagName == "rect") {
        // inside chart
        const index = bisectCenter(date, xScale.invert(pointer(event)[0]));
        ...
        let xOffset;
        if (textWidth + xScale(allData[index].date) >= Width) {
          xOffset = -textWidth;
        } else if (xScale(allData[index].date) - textWidth <= Margin.left) {
          xOffset = 0;
        } else {
          xOffset = -textWidth / 2;
        }
    ...
    const brush = brushX()
      ...
      .on("end", (event) => {
        ...
        if (!extent) {
          if (!idleTimeout) return (idleTimeout = setTimeout(Idle, 350));
          xScale.domain([4, 8]);
        } else {
